...
*******************************************************************************************
0.0.2相对于0.0.1新增了多变量支持，可以任意变量名，移植性极强，仍然是只要支持printf即可
本来准备用hashmap或者前缀树来遍历变量后来觉得就这么点变量不需要复杂的数据结构
0.0.1所有语句调通但是0.0.2仍然在开发中所以有很多bug。。。。
本解释器目标是作为一种高移植性，能进行科学运算的语言。这一个版本只是跑通基本语法
敬请期待后续
...
int if_variable_existed(char *name)
{
...
void set_variable_int(char *name, int value)
{
    if (var_mem_ptr >= 0 && var_mem_ptr <= MAX_VARNUM)
    //printf("input val:%d\n",value);
    {
        int t = if_variable_existed(name);
        if (t != 1013)
        {
            VAR_NAME v_n;
            VARIANT val;
